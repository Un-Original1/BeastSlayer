...
public class EntitySandy extends EntityTameable implements IMagicUser, IEntityMultiPart {
    ...
    public static final ResourceLocation LOOT = new ResourceLocation(AncientBeasts.MODID, "entities/Sandmonster");
    ...
    public void onLivingUpdate()
    {
        ...
        if(EntitySandy.this.isSitting()){
            ...
            this.sandyPartBody.setLocationAndAngles(this.posX + (double) (f2 * 0.8F), this.posY, this.posZ - (double) (f1 * 0.8F), 0.0F, 0.0F);
            this.sandyPartBody2.onUpdate();
            this.sandyPartBody2.setLocationAndAngles(this.posX + (double) (f2 * 1.4F), this.posY, this.posZ - (double) (f1 * 1.9F), 0.0F, 0.0F);
            this.sandyPartBody3.onUpdate();
            this.sandyPartBody3.setLocationAndAngles(this.posX + (double) (f2 * 0.8F), this.posY, this.posZ - (double) (f1 * 2.6F), 0.0F, 0.0F);
            this.sandyPartTail1.onUpdate();
            this.sandyPartTail1.setLocationAndAngles(this.posX - (double) (f2 * 0.6F), this.posY, this.posZ - (double) (f1 * 2.9F), 0.0F, 0.0F);
            this.sandyPartTail2.onUpdate();
            this.sandyPartTail2.setLocationAndAngles(this.posX - (double) (f2 * 1.2F), this.posY, this.posZ - (double) (f1 * 2.1F), 0.0F, 0.0F);
        }
        else {
            this.sandyPartMain.onUpdate();
            this.sandyPartMain.setLocationAndAngles(this.posX, this.posY, this.posZ, 0.0F, 0.0F);
        ...
        if(!this.isBuried() && !this.world.isRemote) {
            this.collideWithPart(this.world.getEntitiesWithinAABBExcludingEntity(this, this.sandyPartMain.getEntityBoundingBox()));
    ...
    {
        double d0 = (this.sandyPartMain.getEntityBoundingBox().minX + this.sandyPartMain.getEntityBoundingBox().maxX) / 2.0D;
        double d1 = (this.sandyPartMain.getEntityBoundingBox().minZ + this.sandyPartMain.getEntityBoundingBox().maxZ) / 2.0D;
        ...
        for (Entity entity : list)
        {
            if (entity instanceof EntityLivingBase)
            {
                ...
                entity.addVelocity(d2 / d4 / 10.0D, 0.0D, d3 / d4 / 10.0D);
            }
        }
    }
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand)
    {
        ...
        if (this.isTamed())
        {
            if (!itemstack.isEmpty())
            {
                if (itemstack.getItem() instanceof ItemFood)
                {
                    ...
                    if (itemstack.getCount() == 64 && this.getHealth() < 120.0F &&  TAME_ITEMS.contains(itemstack.getItem()))
                    {
        ...
        else if (itemstack.getItem() instanceof ItemFood && itemstack.getCount() == 64 && !this.isTamed() &&  TAME_ITEMS.contains(itemstack.getItem()))
        {
            if (!player.capabilities.isCreativeMode)
            {
                itemstack.shrink(64);
    ...
    class SummonSandShooter extends UseMagic<EntitySandy>
    {
        ...
        protected void useMagic() {
            for(int i = 0; i < 3; ++i) {
                ...
                EntitySandShooter shooter = new EntitySandShooter(EntitySandy.this.world, EntitySandy.this);
                shooter.moveToBlockPosAndAngles(blockpos, 0.0F, 0.0F);
                shooter.setCaster(EntitySandy.this);
                EntitySandy.this.world.spawnEntity(shooter);
            }
        }
    ...
    @Override
    public boolean getCanSpawnHere() { return this.world.isThundering() && rand.nextInt(AncientBeastsConfig.sandmonsterSpawnChance) == 0; }
    ...
    @Override
    public SoundEvent getMagicSound() { return null; }
