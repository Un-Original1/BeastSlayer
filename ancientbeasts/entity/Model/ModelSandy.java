...
public class ModelSandy extends ModelBase {
	...
	private ModelSandy.State state = State.NORMAL;
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
		...
		if(this.state != ModelSandy.State.SITTING) {
			this.body5.rotateAngleX = -0.06F + MathHelper.cos(ageInTicks * 0.05F * (float) Math.PI) * 0.05F;
			this.body6.rotateAngleX = -0.02F -MathHelper.cos(ageInTicks * 0.05F * (float) Math.PI) * 0.05F;
			this.tail.rotateAngleX = -0.01F + MathHelper.cos(ageInTicks * 0.05F * (float) Math.PI) * 0.05F;
			this.tail2.rotateAngleX = 0.09F -MathHelper.cos(ageInTicks * 0.05F * (float) Math.PI) * 0.025F;

			this.body4.rotateAngleZ = 0.0F;
			this.body4.rotateAngleY = MathHelper.cos(limbSwing * 0.2F) * limbSwingAmount *  0.3F;
			this.body5.rotateAngleZ = -MathHelper.cos(limbSwing * 0.2F) * limbSwingAmount * 0.6F;
			this.body6.rotateAngleZ = MathHelper.cos(limbSwing * 0.2F) * limbSwingAmount * 0.9F;
			this.tail.rotateAngleZ = -MathHelper.cos(limbSwing * 0.2F) * limbSwingAmount * 0.6F;
			this.tail2.rotateAngleZ = MathHelper.cos(limbSwing * 0.2F) * limbSwingAmount * 0.3F;
		}
