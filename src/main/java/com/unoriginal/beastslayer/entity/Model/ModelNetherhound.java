package com.unoriginal.beastslayer.entity.Model;// Made with Blockbench 4.0.5
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
		...
		if(entityIn instanceof EntityNetherhound) {
			...
			if (netherhound.isSprinting()) {
					this.tail.rotateAngleX = 22.5F * (float) Math.PI / 180F;
					this.head.rotateAngleX = 72.5F * (float) Math.PI / 180F;
					this.head.rotateAngleY = 0F;
					this.leg_fl.rotateAngleX = MathHelper.sin(ageInTicks / 2.5F) * 17.5F * (float) Math.PI / 180F;
			}
