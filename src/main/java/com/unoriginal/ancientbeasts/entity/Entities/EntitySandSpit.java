...
public class EntitySandSpit extends Entity implements IProjectile {
    public EntitySandShooter owner;
    ...
    public EntitySandSpit(World worldIn, EntitySandShooter owner)
    {
        super(worldIn);
        this.owner = owner;
    ...
    {
        if (rayTraceResult.entityHit != null && this.owner != null && !(rayTraceResult.entityHit instanceof EntitySandSpit || rayTraceResult.entityHit == this.owner.getCaster()))
        {
            rayTraceResult.entityHit.attackEntityFrom(DamageSource.causeIndirectDamage(this, this.owner.getCaster()).setProjectile(), 6.0F);
        }
    ...
    private void restoreOwnerFromSave()
    {
        if (this.ownerNbt != null && this.ownerNbt.hasUniqueId("OwnerUUID"))
        {
            ...
            for (EntitySandShooter sandShooter : this.world.getEntitiesWithinAABB(EntitySandShooter.class, this.getEntityBoundingBox().grow(15.0D)))
            {
                if (sandShooter.getUniqueID().equals(uuid))
                {
                    this.owner = sandShooter;
                    break;
                }
            }
        }
