...
public class ItemShieldBook extends ItemBase{
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase target, EnumHand hand) {
        ...
        if (!world.isRemote && !player.getCooldownTracker().hasCooldown(this)) {
            EntityBeam beam = new EntityBeam(world, player, target,280 + AncientBeastsConfig.sBTimerBonus * 2);
            target.addPotionEffect(new PotionEffect(ModPotions.SHIELDED, 280 + AncientBeastsConfig.sBTimerBonus * 2, 0));
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer player, EnumHand hand)
    {
        ...
        World world = player.getEntityWorld();
        if (!world.isRemote && !player.getCooldownTracker().hasCooldown(this)) {
            EntityBeam beam = new EntityBeam(world, player, player, 120 + AncientBeastsConfig.sBTimerBonus);
            player.addPotionEffect(new PotionEffect(ModPotions.SHIELDED, 120 + AncientBeastsConfig.sBTimerBonus, 0));
            world.spawnEntity(beam);
            player.getCooldownTracker().setCooldown(this, 400);
            world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.EVOCATION_ILLAGER_PREPARE_ATTACK, SoundCategory.PLAYERS, 1.0F, 1.0F);
            stack.damageItem(1, player);
