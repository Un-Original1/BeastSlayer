    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if (world.isAirBlock(pos))
        {
            if (!player.getCooldownTracker().hasCooldown(ModItems.ICE_WAND) && !player.getCooldownTracker().hasCooldown(ModItems.ICE_WAND_RED) && !world.isRemote) {
                for (int k = 0; k < 10; ++k) {
                    float f = (float) MathHelper.atan2((double)pos.getZ() - player.posZ, (double)pos.getX() - player.posX);
                ...
                player.getCooldownTracker().setCooldown(ModItems.ICE_WAND, AncientBeastsConfig.frostWandCooldown);
                player.getCooldownTracker().setCooldown(ModItems.ICE_WAND_RED, AncientBeastsConfig.frostWandCooldown);
