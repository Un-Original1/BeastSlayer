...
public class EntityGiant extends EntityMob {
    public static final ResourceLocation LOOT = new ResourceLocation(AncientBeasts.MODID, "entities/Giant");
    ...
    protected void applyEntityAttributes()
    {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(180.0D + AncientBeastsConfig.GiantHealthBonus);
    ...
    public void onLivingUpdate() {
        ...
        if(this.isBeingRidden())
        {
            for(Entity e : this.getPassengers())
            {
                if(e instanceof EntityLivingBase)
                {
                    ...
                    l.attackEntityFrom(DamageSource.causeMobDamage(this), 4.0F);
                }
            }
        ...
        if(this.getAttackTarget() != null && !this.world.isRemote && actionCooldown <= 0 && rand.nextInt(12) == 0 && !this.isBeingRidden()){
            EntityLivingBase target = this.getAttackTarget();
            if(this.getDistanceSq(target) > 18D && !this.didThrow){
    ...
    static class EntityAIGiantMelee extends EntityAIAttackMelee
    {
        ...
        {
            return (this.attacker.width * 1.6F * this.attacker.width * 1.6F + attackTarget.width);
        }
    }
    ...
    private void throwBoulder(EntityLivingBase target){
        ...
        double d1 = target.getEntityBoundingBox().minY + (double)(target.height / 3.0F) - boulder.posY;
        double d2 = target.posZ - this.posZ;
        float f = MathHelper.sqrt(d0 * d0 + d2 * d2) * 0.2F;
        boulder.shoot(d0, d1 + (double)f, d2, 1.0F, 3.0F);
