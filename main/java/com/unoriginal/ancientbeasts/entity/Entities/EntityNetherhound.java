...
public class EntityNetherhound extends EntityTameable {
    public static final ResourceLocation LOOT = new ResourceLocation(AncientBeasts.MODID, "entities/Netherhound");
    ...
    protected void initEntityAI()
    {
        ...
        this.tasks.addTask(5, new EntityAIAttackMelee(this, randSpeed(), true));
        ...
        if((this.getAttackTarget() != null && this.getDistanceSq(this.getAttackTarget()) < 3D && this.rand.nextInt(7) == 0) || --this.flameTicks <= 0){
            ...
            if(!list.isEmpty() || --this.flameTicks <= 0){
                for(EntityLivingBase l : list){
                    if(!this.isOnSameTeam(l) && !world.isRemote){
                        l.setFire(4);
                        l.attackEntityFrom(DamageSource.causeMobDamage(this), 3F);
