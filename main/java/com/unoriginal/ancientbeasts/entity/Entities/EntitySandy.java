...
public class EntitySandy extends EntityTameable implements IMagicUser, IEntityMultiPart, IMob {
    ...
    protected void applyEntityAttributes()
    {
        ...
        if(this.isTamed()){
            this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(120.0D + AncientBeastsConfig.SandyTamedHealthBonus);
        }
        else {
            this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(120.0D + AncientBeastsConfig.SandyHealthBonus);
        }
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand)
    {
        ...
        if (this.isTamed())
        {
            if (!itemstack.isEmpty())
            {
                if (itemstack.getItem() instanceof ItemFood)
                {
                    ...
                    if (itemstack.getCount() == AncientBeastsConfig.sandmonsterTameStackSize && this.getHealth() < 120.0F &&  TAME_ITEMS.contains(itemstack.getItem()))
                    {
                        if (!player.capabilities.isCreativeMode)
                        {
                            itemstack.shrink(AncientBeastsConfig.sandmonsterTameStackSize);
                        }
        ...
        else if (itemstack.getItem() instanceof ItemFood && itemstack.getCount() == AncientBeastsConfig.sandmonsterTameStackSize && !this.isTamed() &&  TAME_ITEMS.contains(itemstack.getItem()))
        {
            if (!player.capabilities.isCreativeMode)
            {
                itemstack.shrink(AncientBeastsConfig.sandmonsterTameStackSize);
    ...
    @Override
    public boolean getCanSpawnHere() { return super.getCanSpawnHere() && this.world.isThundering() && rand.nextInt(AncientBeastsConfig.sandmonsterSpawnChance) == 0 && this.world.getDifficulty() != EnumDifficulty.PEACEFUL; }
