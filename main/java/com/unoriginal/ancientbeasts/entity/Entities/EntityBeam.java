...
public class EntityBeam extends Entity {
    ...
    public EntityBeam(World worldIn, EntityLivingBase casterIn, EntityLivingBase targetIn, int LifeTicksIn)
    {
        this(worldIn);
        this.preventEntitySpawning = true;
        this.setSize(0.8F, 1.4F);
    ...
    public void onUpdate() {
        ...
        if(this.world.isRemote){
            if(this.buffedEntity != null){
                for (int i = 0; i < 2; ++i) {
                    this.world.spawnParticle(ModParticles.SHIELDED_EVIL, this.posX, this.posY + 0.5D, this.posZ, 0D, 0D, 0D);
                }
            } else {
                for (int i = 0; i < 2; ++i) {
                    this.world.spawnParticle(ModParticles.SHIELDED, this.posX, this.posY + 0.5D, this.posZ, 0D, 0D, 0D);
                }
            }
        }
    }
